...
_Note: You can run just the front-end with `next dev`, but `npm run dev` also handles running your API endpoints (located in the `/functions` directory). To support deployment to Netlify we use Netlify serverless functions (rather than Next.js' built-in API functionality) and a custom dev server to make them available to the front-end._
